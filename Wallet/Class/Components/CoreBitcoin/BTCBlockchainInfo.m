...
#ifdef DEBUG // 开发
static NSString const *Blockchain_Url = @"https://testnet.blockchain.info";
#else
static NSString const *Blockchain_Url = @"https://blockchain.info";
#endif
...
- (NSMutableURLRequest*) requestForUnspentOutputsWithAddresses:(NSArray*)addresses {
    ...
    NSString* urlstring = [NSString stringWithFormat:@"%@/unspent?active=%@", Blockchain_Url,[[addresses valueForKey:@"base58String"] componentsJoinedByString:@"%7C"]];
...
- (NSMutableURLRequest*) requestForTransactionBroadcastWithData:(NSData*)data {
    ...
    NSString* urlstring = [NSString stringWithFormat:@"%@/pushtx",Blockchain_Url];
    NSMutableURLRequest* request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:urlstring]];
