...
-(UIFont *)titleFont{
    if (!_titleFont) {
        _titleFont = [UIFont boldSystemFontOfSize:15.0f];
    }
    return _titleFont;
}
- (UIFont *)messageFont {
...
-(void)setup
{
    ...
    NSLayoutConstraint *contentView_CY = [NSLayoutConstraint constraintWithItem:contentView attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1 constant:-60];
    ...
    if(_buttonTitleList.count > 1)
    {
        ...
        NSString *formate_H = [NSString stringWithFormat:@"H:|-%f-[lineView1(%f)]",btnWidth,0.4f];
    ...
    NSDictionary *paramDic = @{@"topMargin":@(kMarginTopButtom),@"lineHeight":@(0.5),@"msgMarginLeftRight":@(17.0f),@"marginTop":@(kMarginTopButtom),@"marginLeft":@(kMarginLeftRight)};
